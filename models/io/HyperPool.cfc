component {
	...
	HyperPool function configure( required cbelasticsearch.models.Config config ){
		...
		lock type="exclusive" name="HyperPoolConfigurationLock" timeout="10" {
			if (
				...
				structKeyExists( configSettings, "clientCertificatePath" ) && len(
					configSettings.clientCertificatePath
				)
			) {
	...
	public Hyper.models.HyperRequest function newRequest( required string route, string method = "GET" ){
		...
		if (
			variables.authenticationScheme == "basic" && node.keyExists( "username" ) && node.keyExists(
				"password"
			)
		) {
	...
	private function getAvailableNode( required string nodeUsage ){
		...
		if ( !structKeyExists( variables.currentIndex, nodeUsage ) ) {
			variables.currentIndex[ nodeUsage ] = 0;
		}
