 ...
 **/
component
	extends  ="LogstashAppender"
	output   ="false"
	hint     ="This a simple implementation of a log appender for Elasticsearch"
	accessors="true"
{
	...
	public any function doRotation() output=false{
		...
		searchBuilder()
			.new( index = getRotationalIndexName() )
			.setQuery( {
				"bool" : {
					"filter" : {
						"range" : {
							"timestamp" : { "lte" : formatter.format( targetDate ).replace( "Z", "+00:00" ) }
						}
					}
				}
			} )
