	...
	singleton
{
	 ...
	 **/
	boolean function indexMappingExists( required string indexName, string mapping ){
	...
	boolean function deleteType( required string indexName, required string type ){
		var searchBuilder = getSearchBuilder().new(
			arguments.indexName,
			arguments.type,
			{ "match_all" : {} }
		);
	...
	any function updateByQuery(
		required SearchBuilder searchBuilder,
