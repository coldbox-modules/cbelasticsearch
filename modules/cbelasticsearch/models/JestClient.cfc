		...
		if( !indexExists( indexDSL.name ) ){
			...
			if( structKeyExists( indexResult[ "index" ], "error" ) ){
				throw( 
					...
					extendedInfo=serializeJSON( indexResult[ "index" ], false, listFindNoCase( "Lucee", server.coldfusion.productname ) ? javacast( "null", 0 ) : "UTF-8" )
				);
			}
	...
	struct function applyMapping( required string indexName, required string mappingName, required struct mappingConfig ){
		...
		if( structKeyExists( mappingResult, "error" ) ){
			...
			throw( 
				...
				extendedInfo=serializeJSON( mappingResult, false, listFindNoCase( "Lucee", server.coldfusion.productname ) ? javacast( "null", 0 ) : "UTF-8" )
			);
	...
	boolean function deleteMapping( required string indexName, required string mapping, boolean throwOnError=false ){
		...
		if( arguments.throwOnError && structKeyExists( deleteResult, "error" ) ){
			throw( 
				...
				extendedInfo=serializeJSON( deleteResult, false, listFindNoCase( "Lucee", server.coldfusion.productname ) ? javacast( "null", 0 ) : "UTF-8" )
			);
		}
	...
	Document function save( required Document document ){
		...
		if( structKeyExists( saveResult, "error" ) ){
			throw( 
				...
				extendedInfo=serializeJSON( saveResult, false, listFindNoCase( "Lucee", server.coldfusion.productname ) ? javacast( "null", 0 ) : "UTF-8" )
			);
		}
	...
	boolean function deleteByQuery( required SearchBuilder searchBuilder ){
		...
		var deleteBuilder = variables.jLoader
										...
										.init( 
											...
											}, false, listFindNoCase( "Lucee", server.coldfusion.productname ) ? javacast( "null", 0 ) : "UTF-8" )
										);
