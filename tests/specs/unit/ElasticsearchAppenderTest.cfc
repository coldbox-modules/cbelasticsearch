component extends="coldbox.system.testing.BaseTestCase" {
	...
	function beforeAll(){
		...
		variables.model = getMockBox().createMock(
			className = "root.modules.cbelasticsearch.models.logging.ElasticsearchAppender"
		);
		makePublic(
			variables.model,
		...
		describe( "Test Elasticsearch logging appender functionality", function(){
			...
			it( "Tests automatic log rotation", function(){
				var documents = getWirebox()
					...
					.execute()
					.getHits();
				var formatter = createObject( "java", "java.text.SimpleDateFormat" ).init(
					"yyyy-MM-dd'T'HH:mm:ssXXX"
				);

				documents.each( function( doc ){
					doc.getMemento()[ "timestamp" ] = formatter
						.format( dateAdd( "d", -50, now() ) )
						.replace( "Z", "+00:00" );
					doc.save();
				} );
